...
public class AdrTreePanel extends JPanel{
    ...
    public AdrTreePanel(List<AdrComponentDescriptor> descriptors) {
    ...
    private void initTree(JPanel parent, List<AdrComponentDescriptor> child) {
        child.forEach(it -> {
            switch (it.getType()){
                case ICON:{
                    parent.add(this.componentsFactory.wrapToSlide(
                            new AdrIconNodePanel((AdrIconDescriptor) it),2,4,2,4));
                    break;
                }
                case PROGRESS_BAR:{
                    parent.add(this.componentsFactory.wrapToSlide(
                            new AdrProgressBarNodePanel((AdrProgressBarDescriptor) it),2,4,2,4));
                    break;
                }
                case GROUP: {
                    JPanel groupPanel = new AdrGroupNodePanel((AdrGroupDescriptor) it);
                    this.initTree(groupPanel,((AdrGroupDescriptor) it).getCells());
                    parent.add(this.componentsFactory.wrapToSlide(groupPanel,2,4,2,4));
                    break;
                }
            }
        });
